...
import { sideBarModes } from "@/lib/utils";
import { Dialog, DialogContent, DialogOverlay } from "@/components/ui/dialog";
import { createPopper } from "@popperjs/core";
import { useMeetingAppContext } from "@/contexts/meeting-app-context-def";
import useMediaStream from "@/hooks/use-media-stream";

import { LucidePictureInPicture2, LucidePictureInPicture } from "lucide-react";
interface PipBTNProps {
  isMobile: boolean;
  isTab: boolean;
}

export function PipBTN({ isMobile, isTab }: PipBTNProps) {
  const { pipMode, setPipMode } = useMeetingAppContext();
  const pipWindowRef = useRef<HTMLVideoElement | null>(null);

  const getRowCount = (length: number) => (length > 2 ? 2 : length > 0 ? 1 : 0);
  const getColCount = (length: number) => (length < 2 ? 1 : length < 5 ? 2 : 3);
  ...
  const togglePipMode = async () => {
    ...
    if ("pictureInPictureEnabled" in document) {
        ...
        try {
          ...
          for (let i = 0; i < rows; i++) {
            for (let j = 0; j < columns; j++) {
              ...
              if (index < videos.length || videos.length === 1) {
                ctx.drawImage(
                  videos[index],
      ...
      pipVideo.addEventListener("leavepictureinpicture", () => {
        pipWindowRef.current = null;
        setPipMode(false);
        if (pipVideo.srcObject instanceof MediaStream) {
          ...
          return (
            <>
              <Popover>
                ...
                <PopoverContent className="w-72 p-4 bg-gray-750">
                  <div className="flex items-center p-3 pb-0">
                  ...
                  <div className="flex flex-col">
                    {webcams.map(({ deviceId, label }, index) => (
                      <button
                        ...
                        className={cn(
                          "px-3 py-1 my-1 pl-6 text-left text-white w-full",
                          deviceId === selectedWebcam.id && "bg-gray-150"
                        )}
                      >
  ...
  return isMobile || isTab ? (
    ...
    >
      ...
      >
        ...
        <DialogContent
          className={cn(
            "fixed inset-0 overflow-y-hidden border-none p-0 bg-transparent", // reset defaults
            "flex items-end justify-end"
          )}
        >
          <div className="w-screen transform overflow-hidden bg-gray-800 shadow-xl transition-all animate-in fade-in slide-in-from-bottom-10 duration-300">
            <div className="container grid bg-gray-800 py-6">
